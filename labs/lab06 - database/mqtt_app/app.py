...
def on_connect(client, userdata, flags, reason_code, properties):
    ...
    client.subscribe("ict720/meowsteam/#")
    client.subscribe("ict720/meowsteam/narodomy/data")
# The callback for when a PUBLISH message is received from the server.
def on_message(client, userdata, msg):
    print(msg.topic+" "+str(msg.payload))
    print("\n\n\nMeow Meow\n\n\n")
    c.execute("SELECT * FROM taist;")
    print(c.fetchall()) 
    c.execute("PRAGMA table_info(taist);")
    print(c.fetchall())  # This should show the inserted rows
    # heartbeat message
    ...
    if msg.topic.split('/')[-1] == "data":
        ...
        db_col.insert_one({"timestamp": datetime.now().isoformat(), 
                           "station": station, 
                           "device": device, 
                           "rssi": rssi})
...
c = conn.cursor()
# c.execute("DROP TABLE IF EXISTS taist")
c.execute('''CREATE TABLE IF NOT EXISTS taist (
          _id INTEGER PRIMARY KEY AUTOINCREMENT,
          timestamp DATETIME DEFAULT TIMESTAMP,
          station TEXT,
          device TEXT,
          rssi INTEGER
          )''')
